...
<div class="main-content">
    <breadcumb :page="'republican_administration'" :folder="title_form" back="true" />
            ...
            <div class="row">
                <b-col md="6">
                    <b-form-group :label="$t('message.management_name_ru')" :invalid-feedback="$t('message.management_name_ru') + ' - '+ $t('message.is_required')">
                        <b-form-input class="mb-2" label="title" v-model="submitData.name_ru" :state="$v.submitData.name_ru.$dirty ? !$v.submitData.name_ru.$error : null" :placeholder="$t('message.enter_management_name')">
                        </b-form-input>
                    </b-form-group>
                    <b-form-group :label="$t('message.management_name_uz')" :invalid-feedback="$t('message.management_name_uz') + ' - '+ $t('message.is_required')">
                        <b-form-input class="mb-2" label="title" v-model="submitData.name_uz" :state="$v.submitData.name_uz.$dirty ? !$v.submitData.name_uz.$error : null" :placeholder="$t('message.enter_management_name')">
                        </b-form-input>
                    </b-form-group>
                    <b-form-group :label="$t('message.management_name_en')" :invalid-feedback="$t('message.management_name_en') + ' - '+ $t('message.is_required')">
                        <b-form-input class="mb-2" label="title" v-model="submitData.name_en" :state="$v.submitData.name_en.$dirty ? !$v.submitData.name_en.$error : null" :placeholder="$t('message.enter_management_name')">
                        </b-form-input>
                    </b-form-group>
                </b-col>
                <b-col md="6">
                    <b-form-group :label="$t('message.email') " :invalid-feedback="$t('message.email') + ' - '+ $t('message.is_required')">
                        <b-form-input class="mb-2" label="title" v-model="submitData.email" :state="$v.submitData.email.$dirty ? !$v.submitData.email.$error : null" :placeholder="$t('message.enter_management_email')">
                        </b-form-input>
                    </b-form-group>
                    <b-form-group :label="$t('message.phone_number')" :invalid-feedback="$t('message.phone_number') + ' - '+ $t('message.is_required')">
                        <b-form-input class="mb-2" label="title" v-model="submitData.phone"   v-mask="['(+998) ##-###-##-##']" :state="$v.submitData.phone.$dirty ? !$v.submitData.phone.$error : null" :placeholder="$t('message.enter_management_phone')">
                        </b-form-input>
                    </b-form-group>
                </b-col>
        ...
        submit() {
            ...
            if (this.$route.params.id) {
                putAdministration(this.submitData, this.$route.params.id).then(() => {
                    ...
                    this.$router.push("/republican-administration/index");
                }).catch(error => {
