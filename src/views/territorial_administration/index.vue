<template>
  <div class="main-content">
    <breadcumb :page="'territorial_administration'" :folder="'list'" />
    <b-card class="wrapper">
      ...
      >
        <div slot="table-actions" class="mb-3">
          <router-link to="/territorial-administration/create">
        ...
        <template slot="table-row" slot-scope="props">
          <span v-if="props.column.field == 'action'">
            <router-link :to="'/territorial-administration/edit/' + props.row.id">
              <i class="i-Eye-Visible text-25 text-success" style="vertical-align: middle"></i>
              {{ props.row.button }}
            </router-link>
            <b-button variant="link" @click="confirmMessage(props.row.id)">
      ...
      <b-modal centered :title="$t('message.delete_title')" size="lg" ref="modal_info" header-bg-variant="danger" header-text-class="text-white" ok-variant="success" :ok-title="$t('message.yes')" :cancel-title="$t('message.no')" @ok="deleteAdmin">
...
<script>
import { getAdministration, deleteAdministration, getAdministrationId } from "../../api/territorial_administration";
...
export default {
  ...
  methods: {
    ...
    deleteAdmin() {
      deleteAdministration(this.delete_id)
        .then(() => {
          this.fetchData();
