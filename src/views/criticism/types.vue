...
<div class="main-content">
    <breadcumb :page="'types_of_applications'" :folder="'list'" />
        ...
        <b-modal id="modal-form" centered :title="$t('message.'+title_form)" ref="modal_form" @hidden="resetModal" @ok="handleOk">
            <form ref="form" @submit.stop.prevent="handleOk">
                <b-form-group :label="$t('message.title')" :invalid-feedback="$t('message.title') + ' - '+ $t('message.is_required')">
                    <b-form-input class="mb-2" label="title" v-model="submitData.name" :state="$v.submitData.name.$dirty ? !$v.submitData.name.$error : null" :placeholder="$t('message.enter_title_type')">
                    </b-form-input>
                </b-form-group>
            </form>
        </b-modal>
...
export default {
    ...
    methods: {
        ...
        handleOk(bvModalEvt) {
            ...
            if (this.edit) {
                putCriticsType(this.submitData, this.submitData.id).then(() => {
                ...
                postCriticsType(this.submitData).then(() => {
                    this.$refs.modal_form.hide()
                    this.fetchData()
                }).catch(error => {
                    for (var k in error.response.data) {
